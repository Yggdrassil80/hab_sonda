...
def getGPSData(ser):
	...
	try:
		...
		#loggerLog.debug("[GPSHelper][getGPSData] baudrate: 9500 timeout 0.5")
		#ser = serial.Serial(port, baudrate = 9600, timeout = 0.5)
		data = ser.readline()
		#loggerLog.info("[GPSHelper][getGPSData] Lectura bruta del puerto: " + data)
	...
	try:
		...
		if data[0:6] == "$GPGGA":
			...
			gpsData[0] = int(0)
                	...
                	gpsData[2] = float(0)
                	gpsData[3] = "KO"
		#loggerLog.info("[GPSHelper][parseGPS_GGA] Fin [0: " + str(gpsData[0]) + "][1: " + str(gpsData[1]) + "][2:" + str(gpsData[2]) + " ][3:" + str(gpsData[3]) + " ]")
		return gpsData
	except Exception:
		e = sys.exc_info()[1]
    		print(e.args[0])
...
def decode(coord):
	...
	try:
		...
		if coord != '':
			...
			coordenada = float(0)
			#loggerLog.debug("[GPSHelper][decode] Fin")
    		return coordenada
	except Exception:
		e = sys.exc_info()[1]
                print(e.args[0])
...
def parseGPS_RMC(data):
		...
		speed = "No valido"
			...
			#Conversion de nudos a m/s
                ...
                loggerLog.info("[GPSHelper][parseGPS_RMC] Fin [0: " + str(gpsDataExtendet[0]) + "][1: " + str(gpsDataExtendet[1]) + "][2:" + str(gpsDataExtendet[2]) + " ][3:" + str(gpsDataExtendet[3]) + "][4:" + time + "][5:" + date + "][6:" + speed)
                return gpsDataExtendet
...
def getGPSDataExtendet(ser):
        ...
        try:
                ...
                #ser = serial.Serial(port, baudrate = 9600, timeout = 0.5)
                data = ser.readline()
                loggerLog.info("[GPSHelper][getGPSDataExtendet] Lectura bruta del puerto: " + data)
